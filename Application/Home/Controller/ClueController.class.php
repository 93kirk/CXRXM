...
class ClueController extends Controller {
    ...
    public function add(){//添加新线索
		$clue = M("clue");
		$rl = M("clue");
		$test['name'] = $_GET['name'];
		$test['create_by'] = $_GET['uname'];
		
		$result = $clue->where($test)->select();
		
		if($result != null){
			$status = 'F';
		    $msg = '该线索名称已被使用，请在后面添加区分标识';
		}
		else{
			$data['name'] = $_GET['name'];
	    	...
	    	$result = $clue->add($data);
			$data1['name'] = $data['name'];
			$result1 = $clue->field('clid')->where($data1)->select();
			$data_rl['uid'] = $_GET['uid'];
			$data_rl['clid'] = $result1[0]['clid'];
			$rl->add($data_rl);
	...
	public function update(){//更新线索信息
		...
		$result = $clue->where($filter)->save($data);
		if(false !== $result&&$result != null){
			    $status = 'T';
		    	$msg = '修改成功';
		    }
	...
	public function showall(){//线索列表
		$user = M("user");
		$filter['crm_user.uid'] = $_GET['uid'];
		$date = $_GET['date'];
		if($date == 1){//最新操作降序
			$result = $user->field('crm_clue.name,crm_clue.clid,crm_clue.update_date as date')
		    ->join('crm_rl_user_clue ON crm_user.uid = crm_rl_user_clue.uid')
		    ->join('crm_clue ON crm_rl_user_clue.cid = crm_clue.cid')->order('date desc')->where($filter)->select();
		}
